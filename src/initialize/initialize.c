...
/*   Updated: 2023/08/07 18:55:24 by cwenz            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	init_game(t_game *game_object, char *map_name)
{
	...
	if (!game_object->hud)
		cleanup_and_exit(game_object, FAIL,
			"Failed to allocate memory for HUD.");
	game_object->mlx = mlx_init(game_object->map->width
			* TILE_PX, game_object->map->height * TILE_PX, "So Long", true);
	if (!game_object->mlx)
